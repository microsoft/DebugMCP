...
## 2. Related Work & Positioning
> **Status: TO DO.** A full related-work review still needs to be conducted and written up. The list below is the starting set of works that **must be covered and paid attention to** — it is a seed, not a finished survey. For each entry: read the paper, summarize its contribution, and articulate precisely how this thesis differs. Also actively search for *additional* significant work not yet listed (e.g., via the "Dissecting the SWE-Bench Leaderboards" survey and Awesome-LLM-SWE-Research), and add it here.

**Works to cover (seed list — expand as needed):**
- **Debug2Fix — "Can Interactive Debugging Help Coding Agents Fix More Bugs?"** (Microsoft, arXiv:2602.18571) — **the most directly related work; review in depth.** Integrates interactive debugging into coding agents via a *subagent architecture* with debuggers for Java & Python; on **GitBug-Java** and **SWE-Bench-Live** it reports **>20% bug-fixing improvement** for some models, and — critically — lets weaker/cheaper models (GPT-5, Claude Haiku 4.5) match or beat stronger ones (Claude Sonnet 4.5). Ablations show both the subagent architecture *and* debugger integration matter. **How we differ (to confirm/refine):** (i) DebugMCP exposes a *real IDE debugger* (VS Code DAP via MCP) rather than a bespoke subagent harness; (ii) we cover **5 languages** (Python, JS/TS, Go, Rust, +.NET stretch) vs. their Java/Python; (iii) we explicitly isolate the effect of the **workflow-instruction skill** (`/debug-live`) as its own arm — i.e., tools-alone vs. tools+skill; (iv) we add an explicit **time** dimension and **budget-capped ROI** analysis. Debug2Fix strongly *motivates* the thesis; our job is independent, cross-language, IDE-debugger confirmation + the skill/time/budget decomposition.
...
**Deliverable for this section:** a written related-work chapter that (a) positions the thesis against each work above, (b) incorporates any newly found significant work, and (c) states the concrete novelty gap DebugMCP fills.
